...
def generate_response(prompt, system_prompt, model="gpt-3.5-turbo"):
    """
    ...
    try:
        ...
        logging.error(f"Authentication failed. Check your {model_type} API key.")
        raise
    except RateLimitError:
        logging.warning("Rate limit exceeded. Retrying after a delay.")
        return "Rate limit exceeded. Please wait and try again."
    except OpenAIError as e:
        logging.error(f"{model_type} API error: {e}")
        return f"An error occurred: {e}"
    except Exception as e:
        logging.critical(f"Unexpected error: {e}")
        raise